<div class="title-link-wrapper mb-3 appear-animate">
    <h2 class="title title-deals mb-1">{{$featuredFlag->name}}</h2>
    <a href="{{url('shop')}}?flag={{$featuredFlag->slug}}" class="font-weight-bold ls-25">View All<i class="w-icon-long-arrow-right"></i></a>
</div>
<div class="swiper-container swiper-theme product-deals-wrapper appear-animate mb-7"
    ...
    @php
        $flagWiseProducts = DB::table('products')
                            ...
                            ->where('products.flag_id', $featuredFlag->id)
    ...
    <div class="swiper-wrapper row cols-lg-5 cols-md-4 cols-2">
        @foreach($flagWiseProducts as $product)
